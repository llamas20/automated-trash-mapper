...
#Built by ChatGPT
class Agent:
    def __init__(self, graph, start_node, hub_node, name=None):
        """
    ...
    def find_nearest_target_edge(self):
        """
        Find the nearest edge with targets from the current node.
    ...
    def step(self):
        ...
        if self.all_targets_collected():
            print("All targets collected. Agent is at the hub.")
        ...
        if not nearest_edge:
            print("No edges with targets remaining.")
            if self.current_node != self.hub_node:
                self.return_to_hub()
            return False  # Simulation ends
        ...
        if self.current_node != edge_start:
            ...
            print(f"Agent moving from {self.current_node} to {edge_start} via path: {path}")
            for node in path[1:]:
                self.current_node = node
                print(f"Agent moved to node {self.current_node}")
    ...
    def traverse_edge(self, node1, node2):
        ...
        print(f"Agent traversing edge from {node1} to {node2}")
        edge_data = self.graph.G[node1][node2]
        targets_collected = edge_data['targets']
        self.collected_targets += targets_collected
        print(f"Collected {targets_collected} targets on edge ({node1}, {node2}). Total collected: {self.collected_targets}")
        ...
        print(f"Agent is now at node {self.current_node}")

    def all_targets_collected(self):
        """Check if all targets on all edges are collected."""
        for edge in self.graph.G.edges(data=True):
            if edge[2]['targets'] > 0:
                return False
        return True
    ...
    def return_to_hub(self):
        ...
        if self.current_node == self.hub_node:
            print("Agent is already at the hub.")
            return

        path = self.find_path(self.current_node, self.hub_node)
        if path:
            print(f"Agent returning to hub via path: {path}")
            for node in path[1:]:
                self.current_node = node
                print(f"Agent moved to node {self.current_node}")
        else:
            print("Agent could not find a path back to the hub.")
